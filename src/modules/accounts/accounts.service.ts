...
// Update the PurchaseOrder interface to include the mdApproval field
interface PurchaseOrder {
  ...
  // ... other fields from the schema.prisma file ...
  mdApproval: string; // Ensure mdApproval is explicitly defined as a string
  poDisputes?: Array<{
    id: string;
    employeeId: string;
    comments: string;
...
export const getBills = async (page = 1, limit = 20) => {
...
export const createBill = async (payload: any) => {
  ...
  const po: any = await prisma.purchaseOrder.findUnique({ where: { id: purchaseOrderId } });
  ...
  const updatedPo = await prisma.purchaseOrder.update({ where: { id: purchaseOrderId }, data: ({ accountBills: [...existing, newBill], salesComments: salesComments ?? po.salesComments } as any) });
...
export const raiseDispute = async (billId: string, employeeId: string, comments: string) => {
  ...
  for (const po of pos) {
    ...
    if (idx !== -1) {
      ...
      await prisma.purchaseOrder.update({ where: { id: po.id }, data: ({ accountBills: bills } as any) });
...
export const raisePoDispute = async (
  ...
  // allow PO disputes only for rejected PO
...
export const addSalesComment = async (poNo: string, comment: string) => {
  // Find the PO by poNo
  const po = await prisma.purchaseOrder.findUnique({ where: { poNo } });
  if (!po) throw new Error("PurchaseOrder not found");

  // Update salesComments
  const updated = await prisma.purchaseOrder.update({
    where: { poNo },
    data: { salesComments: comment },
  });
